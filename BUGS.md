...
### BUG-001 · Polyfill `SlowBuffer` dangereux - Fait 🟢
...
### BUG-002 · Variable globale `createdShare` partagée entre onglets - Fait 🟢
...
### BUG-003 · `Promise.all` sans `await` — Erreurs d'upload perdues - Fait 🟢
...
### BUG-004 · Race condition lors de la suppression de partages - Fait 🟢
...
### BUG-005 · Suppression de partage sans `await` côté frontend - Fait 🟢
...
### BUG-006 · Carte de vitesse d'upload blanche en Dark Mode - Fait 🟢
...
### BUG-007 · Logo sans fallback — Image cassée si fichier absent - Fait 🟢
...
### BUG-008 · Cache-buster du logo trop agressif - Fait 🟢
...
### BUG-009 · `share.removedReason` vérifié AVANT `!share` - Fait 🟢
...
### BUG-010 · `deleteAllFiles` et `remove` utilisent le storage provider actuel, pas celui du share - Fait 🟢
...
### BUG-011 · Fonction `byteToHumanSizeString` dupliquée - Fait 🟢
...
### BUG-012 · Double injection `ConfigService` dans `ShareService` - Fait 🟢
...
### BUG-013 · 17+ warnings ESLint `no-unused-vars` - Fait 🟢
...
### BUG-014 · `streamToUint8Array` privée jamais utilisée - Fait 🟢
...
### BUG-017 · Webhook default title « Pingvin Share » - Fait 🟢
...
### BUG-018 · Textes français hardcodés hors i18n - Fait 🟢
...
### BUG-019 · Meta SEO — `og:description` incorrectement nommée - Fait 🟢
...
### BUG-020 · `deleteTemporaryFiles` synchrone — bloque le event loop - Fait 🟢
...
### BUG-021 · `clearShareTokenCookies` crash si JWT malformé - Fait 🟢
...
### BUG-022 · `mkdirSync` pour créer le dossier de share - Fait 🟢
...
### BUG-023 · Reprise d'upload — données `localStorage` jamais nettoyées si le share expire - Fait 🟢
...
### BUG-024 · Le message d'erreur de refresh token inclut incorrectement les login tokens - Fait 🟢
...
### BUG-025 · `useFsAccessApi={false}` dans Dropzone - Fait 🟢
